...
forks_count int(10) NOT NULL,
watchers_count int(10) NOT NULL,
...
for index_id, key in enumerate(req_dic_items, start=1):
    ...
    sql2 = """INSERT INTO webapi(
    index_id,
    github_id,
    full_name,
    html_url,
    forks_count,
    watchers_count) VALUES(%s, %s, %s, %s, %s, %s)"""

    try:
        cursor2.execute(sql2, (index_id, key["id"], key["full_name"], key["html_url"], key["forks"], key["watchers"]))
        db2.commit()
    except:
        print("Error")
        db2.rollback()
